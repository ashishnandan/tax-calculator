...
object Main {
  ...
  def tax(incomeDetailsList: List[IncomeDetails]): Unit = {
    ...
    for (income <- incomeDetailsList) {
      val taxCalculatorService: TaxCalculatorService = new TaxCalculatorService(income.year, income.age, income.income, income.investment)
      val tax: (Double, Double) = taxCalculatorService.calculateTax
